...
export default function TourHistoryPage() {
  ...
  const fetchBookings = async () => {
    try {
      ...
      const res = await axios.get("https://taxibackend-git-main-bmkl-basnayakes-projects.vercel.app/api/bookings/admin/all", {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <h1 className="text-2xl font-bold mb-6">TOUR ROUTES GRID VIEW ( REAL TIME )</h1>
              ...
              {bookings.map((b, idx) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 border-b text-sm">{b.pickup || "N/A"}</td>
                    <td className="px-4 py-3 border-b text-sm">{b.drop || "N/A"}</td>
                    <td className="px-4 py-3 border-b text-sm text-center">{b.distance_km ?? 0}</td>
                    <td className="px-4 py-3 border-b text-sm text-center">{b.time_minutes ?? 0}</td>
                    <td className="px-4 py-3 border-b text-sm font-semibold text-green-600">Rs {b.estimated_fare ?? 0}</td>
                    <td className="px-4 py-3 border-b text-sm">
                      <span
                        className={`px-3 py-1 rounded-full text-xs font-semibold ${
                          b.status === 'ONGOING'
                            ? 'bg-blue-100 text-blue-700'
                            : b.status === 'DRIVER_PENDING'
                            ? 'bg-yellow-100 text-yellow-700'
                            : b.status === 'CANCELLED'
                            ? 'bg-red-100 text-red-700'
                            : b.status === 'NO_DRIVER_AVAILABLE'
